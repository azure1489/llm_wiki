...
/// Bind host for the local API server. Resolution order: `LLM_WIKI_API_HOST`
/// env (kept for pinned multi-instance deployments, e.g. docker-compose) →
/// the shared `server_bind` resolution (`LLM_WIKI_BIND_HOST` env, then the
/// "Allow LAN access" app setting) → loopback — since the bundled MCP/HTTP
/// API is token-gated but the surface is still best kept off public
